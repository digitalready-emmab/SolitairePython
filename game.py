...
class MyGame(arcade.Window):
    ...
    def on_mouse_release(self, x: float, y: float, button: int,
        ...
        pile = arcade.get_closest_sprite(self.held_cards[0], self.pile_mat_list)
        pile = pile[0]
        reset_position = True
        ...
        if arcade.check_for_collision(self.held_cards[0], pile):
            ...
            # Is it on a middle play pile?
                ...
                if len(self.piles[pile_index]) > 0:
                    ...
                    if CARD_VALUES.index(self.held_cards[0].get_value()) + 1 == 13: 
            ...
            # Release on top play pile? And only one card held?
                ...
                if len(self.piles[pile_index]) > 0:
                    ...
                    if CARD_VALUES.index(self.held_cards[0].get_value()) + 1 == 1: 
                    # Are there no cards in the middle play pile?
                        reset_position = False
                        for i, dropped_card in enumerate(self.held_cards):
                            # Move cards to proper position
    ...
    ## METHODS BY US
    def legal_move(self, top_card):
        """ Check legality of move the player wants to make """
        assert type(top_card) == Card, "Top card not of type card"
        card = self.held_cards[0]
        card_suit = card.get_suit()
        top_card_suit = top_card.get_suit()
        card_value = CARD_VALUES.index(card.get_value()) + 1
        top_card_value = CARD_VALUES.index(top_card.get_value()) + 1
        # checks if it is possible to play suit on the other
        if card_suit == "Hearts" or card_suit == "Diamonds":
            if top_card_suit == "Hearts" or top_card_suit == "Diamonds":
                print("Wrong suit")
                print("Suit:", card_suit)
                return False
        elif card_suit == "Spades" or card_suit == "Clubs":
            if top_card_suit == "Spades" or top_card_suit == "Clubs":
                print("Wrong suit")
                print("Suit:", card_suit)
                return False
        
        if card_value != top_card_value - 1:
            print("Wrong value")
            return False
